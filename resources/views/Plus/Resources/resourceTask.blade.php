...
@section('body')
    ...
    <!-- ==================================== List of tasks is progress ======================================= -->		
                <div class="text-center col-md-11 mx-auto my-2 p-0" style="font-size: 0.9em">                                  
                    <form method="POST" action="/plus/resource/update">
                    	{{ csrf_field() }}
                        <table class="table col-md-10 table-borderless mx-auto">
                            <tr>
                                <td class="py-1 "><strong>Village: <a href="https://{{Session::get('server.url')}}/position_details.php?x={{$task['x']}}&y={{$task['y']}}" target="_blank">{{$task['player']}} ({{$task['village']}})</a></strong></td>
                            ...
                            <tr>
                                ...
                                <td class="py-1  text-info"><strong>Your Contribution:</strong> {{number_format($player)}}</td>
                            </tr>
                            <tr>
                                <td><button class="btn btn-warning px-5" name="update" value="{{$task['task_id']}}"><strong>Submit</strong></button></td>
                                <td class="py-1 "><strong>Comments:</strong> {{$task['comments']}}</td>
                            </tr>
                        </table>
                    </form>
                </div>
